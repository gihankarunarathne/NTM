...
class monthdelta:
    ...
    def __init__(self, months=1):
        if not isinstance(months, int):
            raise TypeError(_('months must be an integer'))
        if abs(months) > 99999999:
            raise OverflowError(_('months must have magnitude <= 99999999'))
    ...
    def __add__(self, other):
        ...
        if isinstance(other, date):
            ...
            try:
                ...
                raise OverflowError(_('date value out of range'))
        return NotImplemented
    ...
    def __ifloordiv__(self, other):
        ...
        if isinstance(other, monthdelta):
            raise TypeError(_('in-place division of a monthdelta requires an integer divisor'))
        if isinstance(other, int):
            return monthdelta(self.__months // other)
        return NotImplemented
    def __neg__(self):
...
def monthmod(start, end):
    ...
    if not (isinstance(start, date) and isinstance(end, date)):
        raise TypeError(_('start and end must be dates'))
