...
class MTraffic():
    ...
    def make_from_db(ntm):
        ...
        res = ntmtools.read_db_var(conn, "traffic.active")  # 0:Disable; Other:Enable;
        try:
            active = (int(res) != 0)
        except:
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.active' " + _("or is not stored. Default value") + " '1'")
        ...
        try:
            ...
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.limit' " + _("or is not stored. Default value") + " '98MB'")
            ntmtools.set_db_var(conn, "traffic.limit", str(98*1024*1024))
            traffic_limit = 98*1024*1024

        res = ntmtools.read_db_var(conn, "traffic.auto_disconnect")  # 0:Disable; Other:Enable;
        try:
            auto_disconnect = (int(res) != 0)
        except:
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.auto_disconnect' " + _("or is not stored. Default value") + " '1'")
        ...
        try:
            ...
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.period_length' " + _("or is not stored. Default value") + " '" + _("Daily") + "'")
            period_length = 0  # 0 -> Daily
            ntmtools.set_db_var(conn, "traffic.period_length", str(int(period_length)))

        val_str = ntmtools.read_db_var(conn, "traffic.custom_days")
        try:
            custom_days = int(val_str)
        except:
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.custom_days' " + _("or is not stored. Default value") + " 30")
            custom_days = 30
            ntmtools.set_db_var(conn, "traffic.custom_days", str(int(custom_days)))

        val_str = ntmtools.read_db_var(conn, "traffic.first_day")
        try:
            first_day = ntmtools.str_to_date(val_str)
        except:
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.first_day' " + _("or is not stored. Default value") + " '2009-10-01'")
        ...
        try:
            ...
            ntmtools.dbg_msg(_("Wrong value for the param") + " 'traffic.period_autoupdate' " + _("or is not stored. Default value") + " '" + _("True") + "'")
    ...
    def load_traffic(conn, start_period, end_period):
        ntmtools.dbg_msg("mtraffic.load_traffic")
        if conn == None:
            ntmtools.dbg_msg('Error in NtmTraffic.loadTraffic: conn is None.', 1000)
            return
        ...
        ntmtools.dbg_msg("start_period, end_period = {0} - {1}".format(start_period, end_period))
        trafficRecUsed = 0
        trafficTraUsed = 0

        for r in rows:
            ntmtools.dbg_msg("r={0}".format(r))
            ntmtools.dbg_msg("r[1]={0} - r[2]={1}".format(r[1], r[2]))
            try:
                # (date, recBytes, traBytes)
                trafficRecUsed += r[1]
                trafficTraUsed += r[2]
            except:
                ntmtools.dbg_msg("Error: NtmTraffic.loadTraffic; some erros in the dailytraffic table.")
            ntmtools.dbg_msg("trafficRecUsed, trafficTraUsed = {0} - {1}".format(trafficRecUsed, trafficTraUsed))

        return (trafficRecUsed, trafficTraUsed)
    load_traffic = staticmethod(load_traffic)
    ## - ##
    ...
    def update_period(self, timestamp):
        ...
        if self.period_autoupdate:
            ...
            if ch:
                ntmtools.set_db_var(self.db_conn, "traffic.first_day", self.first_day.isoformat())
                ret = MTraffic.load_traffic(self.ntm.db_conn, self.first_day, last_day)
                self.rec_traffic = ret[0]
                self.tra_traffic = ret[1]
        ...
        if (total >= self.traffic_limit) and (self.last_state == 1):
            if not self.disc_msgDialog:
                ...
                if self.auto_disconnect:
                    msg = _('Traffic Limit Reached') + ", {0}".format(ntmtools.format_bytes(self.traffic_limit)) + ". " + _("Disconnection is done!!") + "\n" + _("Period: {0} to {1}").format(self.first_day.isoformat(), last_day.isoformat())
                    if self.disconnect_handler != None:
                        self.disconnect_handler()
                    showDialog = True
                else:
                    msg = _('Traffic Limit Reached') + ", {0}".format(ntmtools.format_bytes(self.traffic_limit)) + ".\n" + _('Period: {0} to {1}').format(self.first_day.isoformat(), last_day.isoformat())
    ...
    def build_gui(self):
        ...
        self.gui_expander = gtkb.get_object("expander_traffic")
        self.gui_expander.connect("notify::expanded", self.gui_expander_expanded)
        
        self.gui_main_top_label = gtkb.get_object("trafficFrame_topLabel") # label
        #top_label - gui_main_per
    ...
    def update_main_gui(self):
        ...
        self.gui_main_left.set_tooltip_text("{0} bytes".format(int(self.traffic_limit - traf_tot)))
        ...
        self.gui_main_in.set_tooltip_text("{0} bytes".format(int(self.rec_traffic)))

        self.gui_main_out.set_text("{0:.3f}".format(self.tra_traffic / mbyte))
        self.gui_main_out.set_tooltip_text("{0} bytes".format(int(self.tra_traffic)))

        self.gui_main_total.set_text("{0:.0f}".format(self.traffic_limit/mbyte))
        self.gui_main_total.set_tooltip_text("{0} bytes".format(int(self.traffic_limit)))
        ...
        if self.now != None:
            ...
            if period_len_used != 0: estimate_str = ntmtools.format_bytes(traf_tot * period_len_total / period_len_used)
            else: estimate_str = "--"
        else: estimate_str = "--"
    ...
    def update_preferences_gui(self):
        if self.gtkb == None:
            ntmtools.dbg_msg("MTraffic.update_preferences_gui(*): Gui not builded.\n", 100)
            return
    ...
    def set_preferences_from_gui(self):
        ...
        self.set_preferences(active, traffic_limit, auto_disconnect, period_length, custom_days, self.temp_gui_pref_first_day, period_autoupdate)
    ## - ##


    ## + ##
    def set_preferences(self, active, traffic_limit, auto_disconnect, period_length, custom_days, first_day, period_autoupdate):
        ...
        if (self.active != active):
            self.active = active
            ntmtools.set_db_var(self.db_conn, "traffic.active", ntmtools.bool_to_str_int(active))
            update = True

        if (self.traffic_limit != traffic_limit):
            self.traffic_limit = traffic_limit
            ntmtools.set_db_var(self.db_conn, "traffic.limit", str(int(traffic_limit)))
            update = True

        if (self.auto_disconnect != auto_disconnect):
            self.auto_disconnect = auto_disconnect
            ntmtools.set_db_var(self.db_conn, "traffic.auto_disconnect", ntmtools.bool_to_str_int(auto_disconnect))

        update_traffic = False
        if (self.period_length != period_length):
            self.period_length = period_length
            ntmtools.set_db_var(self.db_conn, "traffic.period_length", str(int(period_length)))
            update_traffic = True

        if (self.custom_days != custom_days):
            self.custom_days = custom_days
            ntmtools.set_db_var(self.db_conn, "traffic.custom_days", str(int(custom_days)))
            update_traffic = True

        if (self.first_day != first_day):
            self.first_day = first_day
            ntmtools.set_db_var(self.db_conn, "traffic.first_day", first_day.isoformat())
            update_traffic = True

        if (self.period_autoupdate != period_autoupdate):
            self.period_autoupdate = period_autoupdate
            ntmtools.set_db_var(self.db_conn, "traffic.period_autoupdate", ntmtools.bool_to_str_int(period_autoupdate))
            if self.period_autoupdate:
                self.update_period(self.now)
                update_traffic = True
...
class BandWidthUsage(gtk.DrawingArea):
    ...
    def draw(self, cr, width, height):
        ...
        cr.set_font_size(14)
        cr.move_to(8, 14)
        cr.show_text(ntmtools.format_bytes(self.speed) + "/s")
    ## - ##
