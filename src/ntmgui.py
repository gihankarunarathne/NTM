...
class NtmGui(object):
    ...
    ### + ###
    def compact_main_window(self):
        self.mainWindow.resize(10, 10)
        self.mainWindow.check_resize()
        self.mainWindow.resize_children()
    ### - ###
        
    ### + ###
    # return the radiobutton 
    def add_row(self, table, row, radioButton4Group, name, info, state):
               
        radiob = gtk.RadioButton(radioButton4Group)
        radiob.show()
        table.attach(radiob, 0, 1, row, row+1, xoptions=gtk.SHRINK)
        ...
        label.show()
        table.attach(label, 3, 4, row, row+1, xoptions=gtk.SHRINK)

        return radiob
    ### - ###

    ### + ###
    def device_type_text(self, device_type):
        info_text = ""
        if   device_type == 1: info_text = _("Wired Ethernet")
        elif device_type == 2: info_text = _("802.11 WiFi")
        elif device_type == 3: info_text = _("GSM-based cellular WAN")
        elif device_type == 4: info_text = _("CDMA/IS-95-based cellular WAN")
        else: info_text = _("Device type is unknown") 
    ...
    def make_device_table(self):
        ...
        label = gtk.Label()
        label.set_markup(_("<b>SEL</b>"))
        label.show()
        table.attach(label, 0, 1, 0, 1, xoptions=gtk.SHRINK)

        label = gtk.Label()
        label.set_markup(_("<b>Name</b>"))
        label.show()
        table.attach(label, 1, 2, 0, 1)
        
        label = gtk.Label()
        label.set_markup(_("<b>Info</b>"))
        label.show()
        table.attach(label, 2, 3, 0, 1)
        
        label = gtk.Label()
        label.set_markup(_("<b>State</b>"))
        label.show()
        table.attach(label, 3, 4, 0, 1, xoptions=gtk.SHRINK)

        self.selint_radiobList = {} 

        device_list = self.ntmo.device_list()
        row = 1
        radioButton4Group = None
        for device in device_list:
            info_text = self.device_type_text(device.deviceType)

            if   device.state == 8: state_text = _("ON")
            else: state_text = _("OFF")
    
            radioButton4Group = self.add_row(table, row, radioButton4Group, device.interface , info_text, state_text)
            self.selint_radiobList[radioButton4Group] = device
            row += 1
    ...
    ### + ###
    def nsb_button_tools_hToggled(self, data = None):
        # TODO
        if self.nsb_button_tools.get_active():
            self.binf_viewport.show()
            # If control reaches here, the toggle button is down
        else:
            self.binf_viewport.hide()
            self.mainWindow.resize(10, 10)
    ...
    def report_window_h_delete(self, widget, event, data=None):
    ...
    ## + ##
    def preferences_sel_int_h_clicked(self, data = None):
        self.selint_button_refresh_h_clicked(None)
        self.selint_dialog.show()
    ## - ##

    ## + ##
    def report_first_day_h_clicked(self, data = None):
    ...
    ## + ##
    def report_last_day_h_clicked(self, data = None):
    ...
    ## + ##
    def report_update_h_clicked(self, data = None):
        self.update_report()
    ## - ##

    ## + SelInt ##
    ## + ##
    def selint_dialog_h_delete(self, widget, event, data=None):
        self.selint_dialog.hide()
        return True
    ## - ##

    ## + ##
    def selint_button_refresh_h_clicked(self, data = None):
        newScrolledWin = self.make_device_table()
        self.selint_dialog_alignmentTable.remove(self.selint_scrolledWinTab)
        self.selint_scrolledWinTab = newScrolledWin
        self.selint_dialog_alignmentTable.add(self.selint_scrolledWinTab)
        self.selint_scrolledWinTab.show()
    ## - ##

    ## + ##
    def selint_button_ok_h_clicked(self, data = None):
        for radioB in self.selint_radiobList:
            if radioB.get_active():
                self.entry_interface.set_text(self.selint_radiobList[radioB].interface)
                break
        self.selint_dialog.hide()
    ## - ##

    ## + ##
    def selint_button_cancel_h_clicked(self, data = None):
        self.selint_dialog.hide()
    ## - ##
    ## - SelInt ##
    ...
    ## TryIcon Handler ##
    # Activate
    def try_icon_h_activate(self, data = None):
    ...
    ## Right Click ##
    def try_icon_h_popup_menu(self, widget, button, time, data = None):
        if button == 3:
            if data:
                data.show_all()
    ...
    ## Menu Handlers ##
    def menu_show_win_h_activate(self, widget):
        self.show_main_window()
    ## - ##


    ## Menu Handlers ##
    def menu_report_h_activate(self, widget):
    ...
    ## + ##
    def menu_preferences_h_activate(self, widget):
    ...
    ## + ##
    def menu_info_h_activate(self, widget):
        if self.ntmo.online:
    ...
    def menu_about_h_activate(self, widget):
        ...
        about.set_authors(["Luigi Tullio <tluigi@gmail.com> (http://luigit.altervista.org)"])
        
        about.set_translator_credits(
              "Luigi Tullio <tluigi@gmail.com> (english,italian)\n" 
            + "Renato Peixe <renato@ua.pt> (portuguese)\n"
            + "A. Ullrich <adi-2010@gmx.de> (german)\n"
            + "Marcin Dopieralski <swiatmar@wp.pl> (polish)\n"
            + "César Sánchez <csalonso5@gmail.com> (spanish)\n"
            + "Ainurko <ainurko@gmail.com> (slovak)\n"
            + "Dimitris Styliara <dstilio@yahoo.gr> (greek)\n"
        )

        about.set_documenters([
            "Tester: Luigi Tullio <tluigi@gmail.com>",
            "Tester: johnnymax <johnny.n3xu5@gmail.com>",
            "Tester: Luigi Romagnosi (Hìr0) <xluigi84@hotmail.it>"
        ])

        about.set_artists([
            "Luigi Tullio <tluigi@gmail.com>",
            "Luigi Romagnosi (Hìr0) <xluigi84@hotmail.it>",
            "kofm"
        ])
    ...
    ## + ##
    def menu_quit_h_activate(self, widget):
        if self.quitActive: return
        self.quitActive = True

        dialog = gtk.Dialog(
            "NTM - " + _("Quit"), self.statusIconMenu.get_toplevel(),
    ...
    ## + ##
    def window_main_h_delete(self, widget, event, data=None):
        ntmtools.dbg_msg("ntmgui.window_main_h_delete")
        self.hide_main_window()
        return True
    ## - ##
    ...
    def report_window_h_delete(self, widget, event, data=None):
        ntmtools.dbg_msg("ntmgui.report_window_h_delete")
        self.report_window.hide()
        self.report_window_show = False
        return True
    ## - ##
    ...
    def window_preferences_h_delete(self, widget, event, data=None):
        ntmtools.dbg_msg("ntmgui.window_preferences_h_delete")
        self.windowPreferences.hide()
        self.windowPreferences_Show = False
        return True
    ## - ##
    ...
    ## + ##
    def preferences_delete_data_h_clicked(self, data = None):
        dialog = gtk.Dialog(
            "NTM - " + _("Delete All Data"), self.statusIconMenu.get_toplevel(),
             gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
             (gtk.STOCK_YES, gtk.RESPONSE_YES,
              gtk.STOCK_NO, gtk.RESPONSE_NO)
        )
        #icon = self.report_window.render_icon(gtk.STOCK_DIALOG_WARNING, gtk.ICON_SIZE_DIALOG)
        icon = self.mainWindow.render_icon(gtk.STOCK_DIALOG_WARNING, gtk.ICON_SIZE_DIALOG)
        dialog.set_icon(icon)
    ...
    ## + ##
    def preferences_button_cancel_h_clicked(self, data = None):
        self.windowPreferences.hide()
        self.windowPreferences_Show = False
    ## - ##
    ...
    ## + ##
    def preferences_button_apply_h_clicked(self, data = None):
        ...
        iconfile = self.filechooserbutton_offline.get_filename()
        iconfile_active = self.filechooserbutton_online.get_filename()

        self.ntmo.set_preferences(
    ...
    ## + ##
    def date_window_h_delete(self, widget, event, data=None):
    ...
    ## + ##
    def apply_prop(self, keep_above, opacity):
              ...
              <tbody>
                <tr style="background-color: WhiteSmoke;">
                  <th colspan="5" align="center">''' + _("Daily Traffic") + '''<br>
                    <span class="red" style="font-size: 11px; width: 100%;"><b>''' + _("All value are listed in MB") + '''.</b></span>
                  </th>
                </tr>
    ...
    def set_report_total(self, rep_res):
        ...
        #ntmtools.dbgMsg("ntmgui.set_report_total : data {0}".format(data))
    ## - end-def #
    ...
    ## + ##
    def set_trayicon_file(self, filename):
        if self.last_conn_state == 0:        
            self.statusIcon.set_from_file(filename)
    ## - ##

    ## + ##
    def set_trayicon_active_file(self, filename):
        if self.last_conn_state == 1:        
            self.statusIcon.set_from_file(filename)
    ## - ##
